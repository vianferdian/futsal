...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-slate-900">Ubah Kompetisi</h1>
            <p class="mt-1 text-sm text-slate-500">Ubah data kompetisi: {{ $competition->name }}</p>
        </div>
    </div>

    <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-xs">
        <form action="{{ route('admin.competitions.update', $competition->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6" x-data="{ loading: false }" @submit="loading = true">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $competition->name) }}" required placeholder="Contoh: Liga Futsal Profesional 2026"
                ...
                <div>
                    ...
                    <input type="text" name="short_name" id="short_name" value="{{ old('short_name', $competition->short_name) }}" required placeholder="Contoh: LFP 2026"
                           class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 shadow-xs focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-hidden sm:text-sm">
                    @error('short_name')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="season" id="season" value="{{ old('season', $competition->season) }}" required placeholder="Contoh: 2026"
                           class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 shadow-xs focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-hidden sm:text-sm">
                    @error('season')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="date" name="end_date" id="end_date" value="{{ old('end_date', $competition->end_date->format('Y-m-d')) }}" required
                           class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 shadow-xs focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-hidden sm:text-sm">
                    @error('end_date')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="sm:col-span-2 space-y-3">
                    ...
                    <div class="flex items-center gap-4">
                        @if ($competition->logo)
                            <img class="h-16 w-16 rounded-lg object-cover border border-slate-200" src="{{ asset('storage/' . $competition->logo) }}" alt="">
                        @else
                            <div class="h-16 w-16 rounded-lg bg-blue-100 text-blue-600 flex items-center justify-center font-bold text-xl">
                                {{ strtoupper(substr($competition->name, 0, 3)) }}
                            </div>
                        @endif
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="3" placeholder="Masukkan deskripsi atau keterangan kompetisi..."
                              class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 shadow-xs focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-hidden sm:text-sm">{{ old('description', $competition->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
