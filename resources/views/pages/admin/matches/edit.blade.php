...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-slate-900">Ubah Jadwal Pertandingan</h1>
            <p class="mt-1 text-sm text-slate-500">Ubah detail pertandingan: {{ $match->match_number }}</p>
        </div>
    </div>

    <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-xs">
        <form action="{{ route('admin.matches.update', $match->id) }}" method="POST" class="space-y-6" x-data="{ loading: false }" @submit="loading = true">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <select name="competition_id" id="competition_id" required class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 shadow-xs focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-hidden sm:text-sm bg-white">
                        ...
                        @foreach ($competitions as $comp)
                            <option value="{{ $comp->id }}" {{ old('competition_id', $match->competition_id) == $comp->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" name="round" id="round" value="{{ old('round', $match->round) }}" required placeholder="Contoh: Babak 16 Besar"
                ...
                <div>
                    ...
                    <input type="text" name="group_name" id="group_name" value="{{ old('group_name', $match->group_name) }}" placeholder="Contoh: Grup A"
                           class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 shadow-xs focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-hidden sm:text-sm">
                    @error('group_name')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="home_team_id" id="home_team_id" required class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 shadow-xs focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-hidden sm:text-sm bg-white">
                        ...
                        @foreach ($teams as $team)
                            <option value="{{ $team->id }}" data-competition="{{ $team->competition_id }}" {{ old('home_team_id', $match->home_team_id) == $team->id ? 'selected' : '' }}>
                                {{ $team->name }} ({{ $team->city }})
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="away_team_id" id="away_team_id" required class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 shadow-xs focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-hidden sm:text-sm bg-white">
                        ...
                        @foreach ($teams as $team)
                            <option value="{{ $team->id }}" data-competition="{{ $team->competition_id }}" {{ old('away_team_id', $match->away_team_id) == $team->id ? 'selected' : '' }}>
                                {{ $team->name }} ({{ $team->city }})
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="time" name="kickoff_time" id="kickoff_time" value="{{ old('kickoff_time', substr($match->kickoff_time, 0, 5)) }}" required
                           class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 shadow-xs focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-hidden sm:text-sm">
                    @error('kickoff_time')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="status" id="status" required class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 shadow-xs focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-hidden sm:text-sm bg-white">
                        @foreach (\App\Enums\MatchStatus::cases() as $stat)
                            <option value="{{ $stat->value }}" {{ old('status', $match->status->value) === $stat->value ? 'selected' : '' }}>
                                {{ $stat->label() }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="border-t border-slate-200 pt-6 flex justify-end gap-3">
                <a href="{{ route('admin.matches.index', ['competition_id' => $match->competition_id]) }}" class="inline-flex justify-center items-center rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-xs hover:bg-slate-50">
                    Batal
                </a>
