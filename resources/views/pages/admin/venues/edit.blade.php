...
@section('title', 'Ubah Venue Pertandingan')
...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-slate-900">Ubah Venue</h1>
            <p class="mt-1 text-sm text-slate-500">Ubah data venue: {{ $venue->name }}</p>
        </div>
    </div>

    <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-xs">
        <form action="{{ route('admin.venues.update', $venue->id) }}" method="POST" class="space-y-6" x-data="{ loading: false }" @submit="loading = true">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $venue->name) }}" required placeholder="Contoh: GOR Among Rogo"
                ...
                <div>
                    ...
                    <input type="text" name="city" id="city" value="{{ old('city', $venue->city) }}" required placeholder="Contoh: Yogyakarta"
                           class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 shadow-xs focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-hidden sm:text-sm">
                    @error('city')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="capacity" id="capacity" value="{{ old('capacity', $venue->capacity) }}" min="0" placeholder="Contoh: 5000"
                           class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 shadow-xs focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-hidden sm:text-sm">
                    @error('capacity')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="address" id="address" rows="3" required placeholder="Masukkan alamat lengkap lokasi venue..."
                              class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 shadow-xs focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-hidden sm:text-sm">{{ old('address', $venue->address) }}</textarea>
                    @error('address')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
