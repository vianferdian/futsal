...
@section('title', 'Ubah Pemain Futsal')
...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-slate-900">Ubah Pemain Futsal</h1>
            <p class="mt-1 text-sm text-slate-500">Ubah data pemain: {{ $player->name }}</p>
        </div>
    </div>

    <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-xs">
        <form action="{{ route('admin.players.update', $player->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6" x-data="{ loading: false }" @submit="loading = true">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $player->name) }}" required placeholder="Contoh: Evan Soumilena"
                ...
                <div>
                    ...
                    <input type="number" name="shirt_number" id="shirt_number" value="{{ old('shirt_number', $player->shirt_number) }}" required min="1" max="99" placeholder="Contoh: 10"
                           class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 shadow-xs focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-hidden sm:text-sm">
                    @error('shirt_number')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="position" id="position" required class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 shadow-xs focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-hidden sm:text-sm bg-white">
                        ...
                        @foreach (\App\Enums\PlayerPosition::cases() as $pos)
                            <option value="{{ $pos->value }}" {{ old('position', $player->position->value) === $pos->value ? 'selected' : '' }}>
                                {{ $pos->label() }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="date" name="birth_date" id="birth_date" value="{{ old('birth_date', $player->birth_date ? $player->birth_date->format('Y-m-d') : '') }}"
                           class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 shadow-xs focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-hidden sm:text-sm">
                    @error('birth_date')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="identity_number" id="identity_number" value="{{ old('identity_number', $player->identity_number) }}" placeholder="Nomor identitas resmi"
                           class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 shadow-xs focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-hidden sm:text-sm">
                ...
                <div class="sm:col-span-2 space-y-3">
                    ...
                    <div class="flex items-center gap-4">
                        @if ($player->photo)
                            <img class="h-16 w-16 rounded-full object-cover border border-slate-200" src="{{ asset('storage/' . $player->photo) }}" alt="">
                        @else
            ...
            <div class="border-t border-slate-200 pt-6 flex justify-end gap-3">
                <a href="{{ route('admin.players.index', ['team_id' => $player->team_id]) }}" class="inline-flex justify-center items-center rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-xs hover:bg-slate-50">
                    Batal
                </a>
