...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-slate-900">Administrator Sistem</h1>
            <p class="mt-2 text-sm text-slate-500">Kelola pengguna dengan hak akses penuh ke seluruh data sistem.</p>
        </div>
        <div>
            <a href="{{ route('admin.users.admins.create') }}" class="inline-flex items-center justify-center rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-xs hover:bg-blue-700">
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-xs">
        <form action="{{ route('admin.users.admins.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4 items-center">
            ...
            <div class="flex w-full sm:w-auto gap-2">
                ...
                @if ($search)
                    <a href="{{ route('admin.users.admins.index') }}" class="w-full sm:w-auto inline-flex justify-center items-center rounded-lg border border-transparent px-4 py-2 text-sm font-semibold text-slate-500 hover:text-slate-700">
                        Reset
                    </a>
                @endif
            </div>
    ...
    <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xs">
        @if ($admins->isEmpty())
            <div class="p-8 text-center text-slate-500 bg-white">
                ...
                <p class="mt-4 text-sm font-semibold">Tidak ada Administrator ditemukan</p>
                <p class="mt-1 text-xs text-slate-400">Silakan tambahkan data Administrator atau ubah pencarian Anda.</p>
            </div>
        @else
                            ...
                            <tr>
                                <td class="whitespace-nowrap px-6 py-4 text-sm">
                                    <div class="flex items-center gap-3">
                                        @if ($admin->photo)
                                            <img class="h-9 w-9 rounded-full object-cover" src="{{ asset('storage/' . $admin->photo) }}" alt="">
                                        @else
                                ...
                                <td class="whitespace-nowrap px-6 py-4 text-sm">
                                    <span class="inline-flex items-center rounded-md px-2 py-1 text-xs font-medium border {{ $admin->isActive() ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-red-50 text-red-700 border-red-200' }}">
                                        {{ $admin->isActive() ? 'Aktif' : 'Tidak Aktif' }}
                                    </span>
                                </td>
                                <td class="whitespace-nowrap px-6 py-4 text-right text-sm font-medium">
                                    <div class="flex items-center justify-end gap-3">
                                        <a href="{{ route('admin.users.admins.edit', $admin->id) }}" class="text-blue-600 hover:text-blue-900">Edit</a>
                                        
                                        @if (auth()->id() !== $admin->id)
                                            <form action="{{ route('admin.users.admins.destroy', $admin->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus Administrator ini?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-red-600 hover:text-red-950">Hapus</button>
                                            </form>
                                        @else
                                            <span class="text-slate-300 cursor-not-allowed select-none text-xs">Self (Aktif)</span>
                                        @endif
                                    </div>
                                </td>
                            </tr>
