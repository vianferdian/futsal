...
@section('title', 'Tambah Tim Futsal')
...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-slate-900">Tambah Tim Baru</h1>
            <p class="mt-1 text-sm text-slate-500">Daftarkan tim atau klub futsal baru ke sistem.</p>
        </div>
    </div>

    <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-xs">
        <form action="{{ route('admin.teams.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6" x-data="{ loading: false }" @submit="loading = true">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="Contoh: Bintang Timur Surabaya"
                ...
                <div>
                    ...
                    <input type="text" name="short_name" id="short_name" value="{{ old('short_name') }}" required placeholder="Contoh: BTS"
                           class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 shadow-xs focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-hidden sm:text-sm">
                    @error('short_name')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="city" id="city" value="{{ old('city') }}" required placeholder="Contoh: Surabaya"
                           class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 shadow-xs focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-hidden sm:text-sm">
                    @error('city')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <div class="mt-1 flex items-center gap-2">
                        <input type="color" name="primary_color" id="primary_color" value="{{ old('primary_color', '#000000') }}"
                ...
                <div>
                    ...
                    <div class="mt-1 flex items-center gap-2">
                        <input type="color" name="secondary_color" id="secondary_color" value="{{ old('secondary_color', '#ffffff') }}"
                               class="h-9 w-14 rounded-md border border-slate-300 cursor-pointer">
                        <span class="text-xs text-slate-500">Klik kotak untuk memilih warna</span>
                    </div>
